Wings-Control V2→V1 迁移分析 反串讲
日期：2026-03-12
范围：基于 untalking-analysis.md 详细分析，沿用 untalking-model.md 排版范式

═══════════════════════════════════════════════════════════════

1.1	功能迁移清单
【需求背景】
Wings V2 单体架构（wings.py 直接管控引擎生命周期）迁移至 V1 Sidecar 三容器架构，需逐项确认每个功能模块的保留/删除/新增状态，形成迁移基线。
【需求价值】
量化迁移范围：保留 34 项 + 新增 28 项 = V1 共 62 项功能，已删除 V2 独有 12 项 + V1 主动删除 6 项。
【需求详情】
四类清单：已保留功能（34项）、未迁移功能（12项，V2独有）、已从V1删除功能（6项）、V1新增功能（28项）。
1.1.1	实现设计
已保留功能（34 项）：
...
2   vLLM-Ascend 适配        同上(engine=vllm_ascend)             同上                                  CANN 环境变量，HCCL 配置
...
7   参数合并                _merge_*_params()                    _merge_*_params()                     vLLM/SGLang/MindIE 各自预处理
...
5   物理 GPU/NPU 探测             core/hardware_detect.py           V1 用环境变量
6   单体引擎管理                   core/engine_manager.py           V1 用脚本生成
...
11  diffusers custom op shim     utils/fix_diffusers_custom_op_shim.py  模型特定
12  function_call 测试            test/function_call.py             测试文件

说明: V2 的独立模块 utils/ascend910_patch.py 提供的 wheel 热补丁能力由 Accel initContainer 方案替代。
...
3   多模态路径探测工具          utils/mmgm_utils.py                 HunyuanVideo 路径探测
...
7   Accel initContainer           wings-accel/Dockerfile, install.sh    补丁注入
8   Accel 补丁执行                _ENGINE_PATCH_KEY_MAP                  引擎无关补丁
9   环境变量硬件检测              WINGS_DEVICE, WINGS_DEVICE_COUNT       替代 torch/pynvml
...
17  Shell 日志文件                wings_start.sh tee → /var/log/wings/  5 副本滚动
...
1.2	参数/环境变量对照
【需求背景】
V2 → V1 迁移中需确保关键环境变量完整保留，新增变量有明确用途，废弃变量有删除理由。
【需求价值】
保留约 80 个核心变量、新增约 55 个 Sidecar/Accel/MindIE 变量、删除约 45 个 V2 独有变量。
【需求详情】
按「保留/新增/删除」三大类，分引擎选择、模型、设备、网络、分布式、代理、Accel、MindIE 等子类细分。
1.2.1	实现设计
保留环境变量（核心，约 80 个）：

类别          环境变量                                         用途
────          ──────                                          ────
引擎选择      ENGINE, WINGS_ENGINE                              用户输入引擎 + 解析后最终引擎
模型          MODEL_NAME, MODEL_PATH, MODEL_TYPE                模型标识
设备          WINGS_DEVICE/DEVICE, WINGS_DEVICE_COUNT/          硬件类型/数量/型号
              DEVICE_COUNT, WINGS_DEVICE_NAME
网络          VLLM_HOST_IP, POD_IP, NODE_IPS                    节点通信
分布式        NCCL_SOCKET_IFNAME, HCCL_IF_IP,                   通信接口
              GLOO_SOCKET_IFNAME
DP            VLLM_DP_RPC_PORT, VLLM_NIXL_SIDE_CHANNEL_PORT    DP / NIXL 侧通道
Ray           RAY_PORT, RAY_HEAD_PORT                           Ray 集群
PD            PD_ROLE, PD_DECODE_XXX                            PD 分离
推测解码      ENABLE_SPECULATIVE_DECODE,                         推测解码
              SPECULATIVE_DECODE_MODEL_PATH
稀疏 KV       SPARSE_ENABLE                                     稀疏模式
QAT           LMCACHE_QAT, LMCACHE_QAT_LOSS_LEVEL,             KVCache QAT 压缩
              LMCACHE_QAT_INSTANCE_NUM
代理          PORT, PROXY_PORT, HTTPX_CONNECT_TIMEOUT           代理监听与连接
...
MindIE        MINDIE_HEALTH_HOST                       127.0.0.2         MindIE 健康探针主机
              MINDIE_HEALTH_PORT                       1026              MindIE 健康探针端口
              MINDIE_LONG_CONTEXT_THRESHOLD            8192              US8 长上下文阈值
              MINDIE_DS_DP/SP/CP/TP                    1/8/2/2           四维并行策略
行为          WINGS_SKIP_PID_CHECK                     false             跳过 PID 检查
              WINGS_FORCE_CHAT_TOPK_TOPP               1                 强制 top_k/top_p
              MAX_REQUEST_BYTES                        20MB              请求体上限
Ascend        PYTORCH_NPU_ALLOC_CONF                   expandable...     NPU 内存
              ASCEND_CUSTOM_OPP_PATH                   —                 自定义算子路径
              HCCL_BUFFSIZE                            1024              HCCL 缓冲
              RAY_EXPERIMENTAL_NOSET_ASCEND_RT_...     1                 Ascend Ray
...
分布式        WINGS_MASTER_*           V1 重构为 ManagedProc
              WINGS_WORKER_*
...
2.1	US1 统一对外引擎命令 — 命令的统一映射【继承】
【需求背景】
V2 的 wings.py 通过 config_loader → engine_manager → OOP adapter 基类直接启动子进程。V1 需要解耦为「生成脚本→共享卷→引擎容器执行」模式，同时保持参数翻译层逻辑不变。
【需求价值】
命令生成→启动管道解耦，引擎启动位置从控制层容器移至引擎专用容器，架构更清晰。
【需求详情】
核心改动在 engine_manager 从 subprocess.Popen 改为 build_start_script → 写入 /shared-volume/start_command.sh。参数合并（config_loader 3/4 层合并）和参数翻译（engine_parameter_mapping.json）逻辑 100% 继承。
2.1.1	实现设计
解耦前 (V2) 命令生成→启动链路：
...
无
2.1.3	接口设计
不涉及（命令映射为内部逻辑，对外接口由 US4 覆盖）
2.1.4	数据结构设计
不涉及

═══════════════════════════════════════════════════════════════

2.2	US2 适配四个引擎 — 参数拼接逻辑【继承】
【需求背景】
vLLM/SGLang/MindIE/vLLM-Ascend 四个引擎的启动命令格式、参数语义、配置方式差异大。adapter 层需屏蔽这些差异，对上层提供统一的 build_start_script(params) → str 接口。
【需求价值】
新引擎接入时只需实现一个 build_start_script 函数即可，框架自动完成参数翻译和脚本生成。
【需求详情】
四个适配器详细场景：vLLM CLI 参数拼接（含推测解码、FP8）、SGLang 语义反转（前缀缓存）、MindIE JSON 5 层覆盖、Ascend CANN 环境注入。
2.2.1	实现设计
vLLM 参数拼接：
...
2.3	US3 单机/分布式模式【继承】
【需求背景】
同一套代码需同时支持单机和分布式两种模式。核心差异在于进程启动方式（subprocess→脚本）和分布式协调（Ray/DP），参数与环境变量层逻辑应完全一致。
【需求价值】
V1 与 V2 的 TP 设置、Ray 协调、DP 地址/端口逻辑 100% 一致，仅进程创建方式因 Sidecar 架构而不同。新增崩溃恢复能力（ManagedProc，V1 领先）。
【需求详情】
单机模式 TP 设置、Ray 分布式 Head/Worker 节点、DP 分布式、DeepSeek V3/V32 Ascend DP 特殊处理。
2.3.1	实现设计
单机模式：
...
无
2.3.3	接口设计
分布式模式下 Master/Worker 之间的内部 API（同 US3 untalking-反串讲）：
...
2.4	US4 统一服务化【继承+新增】
【需求背景】
proxy 层的 14 个对外路由全部继承；需要明确未注册路由是否透传引擎端；四个引擎的转发逻辑是否有差异。
【需求价值】
对外接口零变化，用户无感迁移。新增 Health 独立服务、双门限队列、流式重试提升可用性。
【需求详情】
14 个对外路由全部继承，新增 6 项功能（Health 独立服务、MindIE 专用探针、FORCE_TOPK_TOPP、MAX_REQUEST_BYTES、细粒度超时、WINGS_SKIP_PID_CHECK）。
2.4.1	实现设计
...
按路由定义数为 11 个，其中 /health 同时注册了 GET 和 HEAD。
...
未注册 API 的处理行为：
当前实现中未注册路由返回 FastAPI 默认 404，不自动透传引擎端。四个引擎的已注册路由转发逻辑完全相同，Proxy 不区分引擎类型。
...
【需求背景】
需在不修改引擎镜像前提下，动态注入加速补丁。通过 initContainer + 共享卷 + 环境变量注入实现三容器协作。
【需求价值】
补丁独立于引擎镜像更新，维护成本低，接入无侵入。
【需求详情】
四个环节：initContainer 拷贝 → 使能环境变量检测 → 补丁选项注入 start_command.sh → 引擎容器安装并执行。
2.5.1	实现设计
三容器协作流程：
...
2.6	US6 日志汇聚逻辑【重构】
【需求背景】
V2 单容器 wings.py → subprocess.Popen(engine) → 管道汇聚 stdout；V1 三容器各有独立 stdout，需重新设计日志方案。
【需求价值】
通过 K8s 原生 --all-containers 机制 + 统一日志格式 + Speaker 控制 + 噪声过滤，实现等效的日志可观测性。
【需求详情】
三容器日志流、统一日志格式、日志文件持久化、噪声过滤机制。
2.6.1	实现设计
V2 (wings) 原有日志逻辑：
...
日志文件持久化:
Shell 层面 wings_start.sh 通过 exec > >(tee -a "$LOG_FILE") 2>&1 将全部输出同时写入 /var/log/wings/wings_start.log（5 副本滚动），但该路径未挂载持久卷，容器重启后丢失。Python 层面无 RotatingFileHandler，所有日志仅输出到 stderr。

日志噪声过滤：

模块                  过滤内容                                    机制
────                  ────                                        ────
noise_filter.py       /health 探针、Prefill/Decode batch 噪声     logging.Filter + sys.stdout/stderr 包装
speaker_logging.py    多 worker 日志抑制、uvicorn.access           speaker 决策 + _DropByRegex Filter
...
2.7	US7 RAG 二级推理【继承】
【需求背景】
RAG 二级推理完全在 proxy 服务层实现，通过 HTTP 调用引擎 /v1/chat/completions 接口，不依赖引擎特定功能。四个引擎均支持该标准接口。
【需求价值】
100% 继承，8/8 文件完全一致。
【需求详情】
继承状态 8/8 文件一致，仅 fastchat import 改为 try/except 懒加载。
...
引擎无关性: RAG 模块通过 HTTP 调用引擎的 /v1/chat/completions API，不依赖任何引擎特定接口。
...
【需求背景】
DeepSeek 满血模型（DeepseekV3ForCausalLM / DeepseekV32ForCausalLM）在 MindIE 2×8 分布式场景下，当用户输入输出长度总和超过阈值（8K）时，需启用四维并行策略（dp/sp/cp/tp）以支持长上下文推理。
【需求价值】
...
2.8.1	实现设计
MindIE config.json 生成机制对比：

V2 — 直接进程模式:
    mindie_adapter.py 的 _update_mindie_config() 直接读取容器内
    /usr/local/Ascend/mindie/latest/mindie-service/conf/config.json，原地修改后写回。

V1 — Sidecar 模式:
    mindie_adapter.py 的 build_start_script(params) 生成 bash 脚本，
    内嵌 Python 代码通过 JSON merge-update 方式更新 config.json。
    更新数据分为 5 个 overrides dict：
        server_overrides        → ServerConfig
        backend_overrides       → BackendConfig (根级)
...
──────                      ─────────                      ───────                   ──────────────────
INPUT_LENGTH  ─┐
OUTPUT_LENGTH  ─┤─ _merge_mindie_params() ──► params['dp']=1  ──► model_config ──► ModelConfig[0].dp=1
MODEL_NAME     ─┤       ↓                     params['sp']=8  ──► model_config ──► ModelConfig[0].sp=8
DISTRIBUTED    ─┘   检测条件:                  params['cp']=2  ──► model_config ──► ModelConfig[0].cp=2
                    total > 8192               params['tp']=2  ──► model_config ──► ModelConfig[0].tp=2
                    && DeepSeek 架构
                    && distributed
...
环境变量                           默认值     说明
──────                            ────       ────
MINDIE_LONG_CONTEXT_THRESHOLD      8192       长上下文触发阈值
