...
| 模块 | 迁移状态 | 说明 |
|------|---------|------|
| config_loader | 继承+增强 | 新增 US8 长上下文、PCIe 卡检测 |
| engine_manager | 继承 | 别名映射 + importlib 动态导入 |
| hardware_detect | 简化 | 不再依赖 pynvml/torch，纯环境变量驱动 |
| engines/ (3个适配器文件) | 继承 | vllm_adapter / sglang_adapter / mindie_adapter（vllm_ascend 复用 vllm_adapter） |
| proxy/gateway | 继承+拆分 | health 独立为单独进程(:19000) |
| rag_acc/ | 继承 | 100% 复用，与引擎层无关 |
...
graph TB
    subgraph "环境变量分层体系"
        direction TB
        L1["Sidecar 架构变量<br/>SHARED_VOLUME_PATH<br/>START_COMMAND_FILENAME<br/>WINGS_SKIP_PID_CHECK"]
        L2["引擎/模型变量 约80个<br/>ENGINE, MODEL_PATH, TP_SIZE<br/>MAX_MODEL_LEN, DTYPE, QUANTIZATION<br/>VLLM_HOST_IP, NODE_IPS ..."]
        L3["特性开关变量<br/>ENABLE_ACCEL, ENABLE_RAG_ACC<br/>PD_ROLE, SPARSE_ENABLE<br/>MINDIE_LONG_CONTEXT_THRESHOLD"]
    end
    L1 -->|V1 新增| L2
    L2 -->|V2 继承| L3
```
...
| **MindIE** | `MINDIE_LONG_CONTEXT_THRESHOLD` | `8192` | US8 长上下文阈值 |
| | `MINDIE_DS_DP/SP/CP/TP` | `1/8/2/2` | US8 长上下文并行策略 |
...
| **多模态** | `HYV_*`, `SAVE_PATH` | 多模态功能已从 V1 移除 |
