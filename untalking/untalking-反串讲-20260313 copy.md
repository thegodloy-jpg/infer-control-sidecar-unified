...
MaaS 页面需向下发的 YAML 注入以下参数，用于控制 Accel 使能及镜像版本选择：

| 参数 | 说明 | 示例 |
|------|------|------|
| `ENGINE_VERSION` | 引擎镜像版本号，决定 `wings-accel` initContainer 镜像标签 | `v0.1.0` |
...
**② engine 容器**（等待 start_command.sh，执行即可；install.sh 已由 wings-control 内嵌到 start_command.sh 头部）：
...
> **说明**：wings-control 在 `ENABLE_ACCEL=true` 时，会在生成的 `start_command.sh` 头部自动插入：
>
> ```bash
> if [ -f "/accel-volume/install.sh" ]; then
>  cd /accel-volume && bash install.sh && cd -
> fi
...
`wings_entry.py` 中的 `_build_accel_env_line(engine)` 根据引擎类型自动生成 `WINGS_ENGINE_PATCH_OPTIONS`：
...
生成逻辑优先级：
1. 用户通过 `WINGS_ENGINE_PATCH_OPTIONS` 环境变量传入自定义值（JSON 格式校验，非法则回退）
2. 未设置时，按 `_ENGINE_PATCH_KEY_MAP[engine]` + `_DEFAULT_PATCH_FEATURES` 自动生成

输出示例：`export WINGS_ENGINE_PATCH_OPTIONS='{"vllm":["test_patch"]}'`

**② 注入补丁安装脚本调用**

当 `settings.ENABLE_ACCEL=True` 时（由 `settings.py` 中的 `ENABLE_ACCEL` 环境变量驱动），`build_launcher_plan()` 在 `start_command.sh` 头部注入：
...
if [ -f "/accel-volume/install.sh" ]; then
    ...
    echo '[wings-accel] WARNING: /accel-volume/install.sh not found, skipping patch install.'
fi
export WINGS_ENGINE_PATCH_OPTIONS='{"vllm":["test_patch"]}'
# --- 以下为引擎启动命令 ---
...
注入位于 `accel_preamble`，插入顺序：`shebang` → `set -euo pipefail` → `install_snippet` → `WINGS_ENGINE_PATCH_OPTIONS export` → `adapter 生成的引擎命令`。
...
flowchart TD
    ...
    A -->|是| C["生成 install_snippet<br/>检测并执行 /accel-volume/install.sh"]
    C --> D["_build_accel_env_line(engine)<br/>生成 export WINGS_ENGINE_PATCH_OPTIONS"]
    D --> E["accel_preamble = install_snippet + export 语句"]
    B --> F["command = shebang + accel_preamble + script_body"]
    E --> F
    F --> G["写入 /shared-volume/start_command.sh"]
```
...
├── install.sh                  # 一级安装入口：cd wings_engine_patch && bash install.sh
├── supported_features.json     # 特性声明（引擎→版本→补丁列表）
└── wings_engine_patch/
    └── install.sh              # 二级安装：pip install *.whl
```

**① 保证特性脚本可用**

- `Dockerfile` 中显式 `chmod +x` 两个 `install.sh`，确保运行时有执行权限
- `supported_features.json` 声明每个引擎版本支持的补丁列表，供验证和管理使用：
  ```json
  {
    "vllm":   { "0.12.0": ["test_patch"] },
    "sglang": { "0.4.0":  ["test_patch"] },
    "mindie": { "1.0.0":  ["test_patch"] }
  }
  ```
...
```
/accel-volume/install.sh
  → cd wings_engine_patch && bash install.sh
    → pip install *.whl    （在引擎容器的 Python 环境中安装补丁 whl 包）
```

**③ 清晰报错**

- `install.sh` 在 `start_command.sh` 中包裹在 `if [ -f ... ]` 检测里——若 initContainer 未运行或 `accel-volume` 未挂载，输出 WARNING 日志而不是直接 crash
