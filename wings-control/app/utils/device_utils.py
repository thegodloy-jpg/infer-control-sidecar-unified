...
# 状态: ⚠️ 遗留代码 — 本模块为 wings 主项目的设备工具模块移植副本。
#       在 sidecar 架构下，控制容器无法直接访问 GPU/NPU SDK（pynvml/torch_npu），
#       因此所有函数均未被项目中任何其他模块调用（100% 死代码）。
#       实际硬件检测由 app/core/hardware_detect.py 通过环境变量完成。
#       保留此文件仅供参考，后续可安全删除。
#
# 功能概述:
#   本模块提供跨平台的设备探测功能，支持：
