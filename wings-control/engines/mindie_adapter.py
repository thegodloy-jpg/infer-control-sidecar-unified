...
def _build_distributed_env_commands(params: Dict[str, Any]) -> List[str]:
    ...
    # 策略：首选外部预置的 RANK_TABLE_PATH（兼容 V1），降级为动态生成
    #
    # V1 要求外部挂载 rank table 文件并通过 RANK_TABLE_PATH 传入，
    # V2 新增了动态生成能力。为保持向后兼容，优先使用外部文件。
    external_rank_table = os.getenv("RANK_TABLE_PATH", "").strip()
    if external_rank_table and os.path.isfile(external_rank_table):
        # 外部预置路径可用 — 兼容 V1 行为：chmod 640 + 直接引用
        logger.info("[mindie] Using external RANK_TABLE_PATH: %s", external_rank_table)
        rank_table_cmds = [
            f"# ── HCCL rank table (external: {external_rank_table}) ──",
            f"chmod 640 {external_rank_table}",
        ]
        ranktable_path = external_rank_table
    else:
        if external_rank_table:
            logger.warning(
                "[mindie] RANK_TABLE_PATH=%s set but file not found, falling back to dynamic generation",
                external_rank_table,
            )
        # 动态生成单节点 rank table（V2 默认行为）
