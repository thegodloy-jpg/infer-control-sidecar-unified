...
#   - health_router.py  —— 健康状态机核心，持续探测后端 /health + PID 存活
...
#   - proxy_config.py   —— proxy 运行时配置（端口、刷包策略、并发限制等）
...
# "proxy.gateway:app" 和 "proxy.health_service:app" 引用 FastAPI 应用实例。
#
# Sidecar 契约:
...
"""
__all__ = [
    "gateway", "health_router", "health_service",
    "http_client", "proxy_config", "queueing",
    "speaker_logging", "tags"
]