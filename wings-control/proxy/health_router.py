...
# 因此不再依赖 PID 校验，仅以 HTTP 探活 (backend_ok) 判断后端状态。
# pid_alive 字段仍保留在响应中作为诊断信息，但不参与状态机判定。
...
def _advance_state_machine(h: dict, pid_alive: bool, backend_ok: bool) -> None:
    ...
    sidecar 架构下 PID 不可达，仅依据 backend_ok 判定。
    pid_alive 参数保留但不参与判定（兼容旧调用签名）。
    """
    if backend_ok:
        ...
        if first_time_ready:
            C.logger.info(
                ...
                "(pid_alive=%s, backend_ok=%s)",
                pid_alive, backend_ok,
            )
...
def map_http_code_from_state(h: dict) -> int:
    ...
    # sidecar 架构：仅以 backend_ok 判定，pid_alive 不参与
    is_ready = (
        h["backend_ok"] and
        h["ever_ready"] and
        h["status"] == 1
